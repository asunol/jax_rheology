...
#
#   sbatch run_fenep_rset_prod.sh <run_name> <cur|u05> <vel|velp> [EXTRA CLI ...]
#     e.g. sbatch run_fenep_rset_prod.sh fene7_cur_velp    cur velp
#          sbatch run_fenep_rset_prod.sh fene7_u05_vel     u05 vel
#          sbatch run_fenep_rset_prod.sh fene7_cur_velp_s1 cur velp --seed 1
#          sbatch run_fenep_rset_prod.sh fene7_cur_velp_lo cur velp --lam-init 0.35
# ===========================================================================
set -u
RUN=${1:?run name}
RATEKEY=${2:?cur|u05}
...
echo "=== fenep_rset run=$RUN rate=$RATEKEY mode=$MODE extra='$EXTRA'  Job $SLURM_JOB_ID  $(date) ==="
...
python -u campaigns/visco_opt_tbnn_contraction_run.py \
    ...
    --seed 0 $RATE $WP --norms-json "$NORMS" --n-sub 8 --ckpt-every 25 \
    --time-budget-s "$TIME_BUDGET" --out-dir "$OUT" --run-name "$RUN" $EXTRA
EXIT=$?
echo "=== fenep_rset $RUN done  exit $EXIT  $(date) ==="
...
if [ ! -f "$OUT/$RUN/DONE" ] && [ "$EXIT" -eq 0 ]; then
    ...
    if [ "$N" -lt "$MAX_RESUB" ]; then
        ...
        sbatch slurm/run_fenep_rset_prod.sh "$RUN" "$RATEKEY" "$MODE" $EXTRA
    else
