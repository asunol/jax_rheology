#!/usr/bin/env python
"""
Cluster runner: train the instantaneous closure on PIV-like observations.

The PIV variant of the constriction training runner. The training target is
passed through the PIV observation operator first -- window-averaged to
interrogation windows and optionally given correlated, spatially varying
noise -- so the fit sees what a velocimetry measurement would have seen
rather than the full-resolution field. Writes every figure and result to the
run directory for later analysis.

The ``--resolution-piv``, ``--piv-*`` and ``--add-piv-noise`` flags are what
distinguish it from ``run_tbnn_debug_constriction_cluster_new.py``; the rest
of the training options are the same.

USAGE:
    python run_tbnn_debug_constriction_cluster_new_piv.py [iteration_number] [options]
    
EXAMPLES:
    # Basic usage
    python run_tbnn_debug_constriction_cluster_new_piv.py 1
    
    # Custom training configuration
    python run_tbnn_debug_constriction_cluster_new_piv.py 2 --num-steps 20 --learning-rate 5e-4
    
    # With specific initialization
    python run_tbnn_debug_constriction_cluster_new_piv.py 3 --init-method powerlawish_current
    
    # Larger architecture and longer simulation
    python run_tbnn_debug_constriction_cluster_new_piv.py 4 --architecture 64 64 64 --outer-steps 200
    
    # With viscosity regularization
    python run_tbnn_debug_constriction_cluster_new_piv.py 5 --visc-loss --reg-lambda-slope 1e-3 --reg-lambda-curv 3e-4
    
    # With warmup/tail for long simulations
    python run_tbnn_debug_constriction_cluster_new_piv.py 6 --outer-steps 400 --use-warmup-tail --warmup-steps 300 --tail-steps 100
    
    # Save trajectory data and model states
    python run_tbnn_debug_constriction_cluster_new_piv.py 7 --save-traj-info
    
    # With TBNN model options (sigmoid viscosity model)
    python run_tbnn_debug_constriction_cluster_new_piv.py 8 --s-floor 0.7 --alpha-temp 2.0 --global-scalar-lr-scale 10.0
    
    # Freeze eta0 to learn only curvature (forces PDE to learn shape, not magnitude)
    python run_tbnn_debug_constriction_cluster_new_piv.py 9 --freeze-eta0 --eta0-fixed 1.0 --num-steps 10
    
    # Use curvature control to prevent drops at low shear rates (clean fix for f32 issues)
    python run_tbnn_debug_constriction_cluster_new_piv.py 10 --freeze-eta0 --eta0-fixed 1.0 \
        --mu-min-gamma 5e-2 --gate-gamma 5e-2 --gate-width-z 0.5 --num-steps 10
    
    # Enable per-mode power-law bumps for extra expressiveness
    python run_tbnn_debug_constriction_cluster_new_piv.py 11 --freeze-eta0 --eta0-fixed 1.0 \
    ...
    python run_tbnn_debug_constriction_cluster_new_piv.py 13 --num-steps 5 --learning-rate 4e-2 \
    ...
    python run_tbnn_debug_constriction_cluster_new_piv.py 14 --freeze-eta0 --eta0-fixed 1.0 \
    ...
    python run_tbnn_debug_constriction_cluster_new_piv.py 15 --freeze-eta0 --eta0-fixed 1.0 \
    ...
    # Use PIV resolution downsampling (no noise)
    python run_tbnn_debug_constriction_cluster_new_piv.py 16 --num-steps 10 --learning-rate 1e-3 \
        --resolution-piv --piv-W-win 32 16 --piv-overlap 0.75 --piv-kernel hann
    
    # Use PIV resolution + realistic noise (0.5% of U95)
    python run_tbnn_debug_constriction_cluster_new_piv.py 17 --num-steps 10 --learning-rate 1e-3 \
        --resolution-piv --piv-W-win 32 16 --piv-overlap 0.75 \
        --add-piv-noise --piv-noise-p-percent 0.5 --piv-noise-corr-frac 0.35 \
        --piv-noise-beta-grad 0.5 --piv-noise-use-bias --piv-noise-seed 1453
    
    # Full training with PIV + noise (2% noise, 2-stage training)
    python run_tbnn_debug_constriction_cluster_new_piv.py 18 --freeze-eta0 --eta0-fixed 1.0 \
        --two-stage-etainf-then-curv --stage1-steps-etainf 10 --stage2-steps-curv 2 \
        --stage1-etainf-only --stage1-reset-momentum --stage1-early-stop-on-flip \
        --freeze-centers --mu-min-gamma 1e-1 --mu-max-gamma 1e1 \
        --shape-loss --shape-weight 0.5 \
        --resolution-piv --piv-W-win 32 16 --piv-overlap 0.75 \
        --add-piv-noise --piv-noise-p-percent 2.0 \
        --learning-rate 5e-2 --global-scalar-lr-scale 10.0 \
        --visc-loss --reg-s-cap 0.6 --reg-lambda-slope 1e-3 --reg-lambda-curv 3e-4
"""
...
# Import the constriction debugging functions (PIV version)
from jax_rheology.training.instantaneous import debug_one_step_gradient_constriction
...
def main():
    ...
    # PIV resolution and noise options
    parser.add_argument('--resolution-piv', action='store_true', default=False,
                       help='Enable PIV resolution downsampling (default: False)')
    parser.add_argument('--piv-W-win', nargs=2, type=int, default=[32, 16],
                       help='PIV interrogation window size (W_x W_y) in pixels (default: 32 16)')
    parser.add_argument('--piv-overlap', type=float, default=0.75,
                       help='PIV window overlap fraction (default: 0.75)')
    parser.add_argument('--piv-kernel', type=str, default='hann',
                       choices=['hann', 'uniform'],
                       help='PIV smoothing kernel type (default: hann)')
    parser.add_argument('--add-piv-noise', action='store_true', default=False,
                       help='Add realistic PIV noise to reference data (default: False)')
    parser.add_argument('--piv-noise-p-percent', type=float, default=0.0,
                       help='PIV noise level as percent of U95 (default: 0.0, try 0.5, 1.0, 2.0)')
    parser.add_argument('--piv-noise-corr-frac', type=float, default=0.35,
                       help='PIV noise spatial correlation fraction (default: 0.35)')
    parser.add_argument('--piv-noise-beta-grad', type=float, default=0.5,
                       help='PIV noise heteroscedastic strength (default: 0.5)')
    parser.add_argument('--piv-noise-use-bias', action='store_true', default=True,
                       help='Include static bias field in PIV noise (default: True)')
    parser.add_argument('--piv-noise-seed', type=int, default=1453,
                       help='Random seed for PIV noise generation (default: 1453)')
    ...
    output_dir = f"./work/instantaneous_train_piv/iteration_{args.iteration:02d}_{timestamp}"
    ...
    print(f"PIV resolution mode: {args.resolution_piv}")
    if args.resolution_piv:
        print(f"  - PIV window size: {tuple(args.piv_W_win)} pixels")
        print(f"  - PIV overlap: {args.piv_overlap} ({int(args.piv_overlap*100)}%)")
        print(f"  - PIV kernel: {args.piv_kernel}")
        if args.add_piv_noise:
            print(f"  -  PIV noise ENABLED (applied to reference only):")
            print(f"     Noise level: {args.piv_noise_p_percent}% of U95")
            print(f"     Correlation: {args.piv_noise_corr_frac}")
            print(f"     Heteroscedastic beta: {args.piv_noise_beta_grad}")
            print(f"     Static bias: {args.piv_noise_use_bias}")
            print(f"     Random seed: {args.piv_noise_seed}")
        else:
    ...
    try:
        results1 = debug_one_step_gradient_constriction(
            ...
            # PIV resolution and noise options
            resolution_piv=args.resolution_piv,
            piv_W_win=tuple(args.piv_W_win),
            piv_overlap=args.piv_overlap,
            piv_kernel=args.piv_kernel,
            add_piv_noise=args.add_piv_noise,
            piv_noise_p_percent=args.piv_noise_p_percent,
            piv_noise_corr_frac=args.piv_noise_corr_frac,
            piv_noise_beta_grad=args.piv_noise_beta_grad,
            piv_noise_use_bias=args.piv_noise_use_bias,
            piv_noise_seed=args.piv_noise_seed,
            # Model architecture options
            M=args.M,
    ...
    try:
        ...
        with open(summary_file, 'w') as f:
            ...
            f.write(f"PIV resolution and noise:\n")
            f.write(f"  resolution_piv: {args.resolution_piv}\n")
            if args.resolution_piv:
                f.write(f"  piv_W_win: {tuple(args.piv_W_win)}\n")
                f.write(f"  piv_overlap: {args.piv_overlap}\n")
                f.write(f"  piv_kernel: {args.piv_kernel}\n")
                f.write(f"  add_piv_noise: {args.add_piv_noise}\n")
                if args.add_piv_noise:
                    f.write(f"  piv_noise_p_percent: {args.piv_noise_p_percent}\n")
                    f.write(f"  piv_noise_corr_frac: {args.piv_noise_corr_frac}\n")
                    f.write(f"  piv_noise_beta_grad: {args.piv_noise_beta_grad}\n")
                    f.write(f"  piv_noise_use_bias: {args.piv_noise_use_bias}\n")
                    f.write(f"  piv_noise_seed: {args.piv_noise_seed}\n")
